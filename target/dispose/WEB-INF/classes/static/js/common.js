...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', topWindow).each(function () {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                // 显示tab对应的内容区
                $('.mainContent .Keyier_iframe', topWindow).each(function () {
                    if ($(this).data('id') == dataUrl) {
                        $(this).show().siblings('.Keyier_iframe').hide();
                        return false;
                    }
                });
            }
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="Keyier_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
        $('.mainContent', topWindow).find('iframe.Keyier_iframe').hide().parents('.mainContent').append(str1);
